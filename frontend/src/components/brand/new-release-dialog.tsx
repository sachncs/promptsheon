...
export function NewReleaseDialog({ capabilityId }: NewReleaseDialogProps) {
  ...
  const capabilityIdValue = useWatch({ control: form.control, name: 'capabilityId' });
  const capabilityVersionValue = useWatch({ control: form.control, name: 'capabilityVersion' });
  const environmentValue = useWatch({ control: form.control, name: 'environment' });
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          <FieldGroup>
            ...
            <Field label="Version" htmlFor="rel-ver" error={form.formState.errors.capabilityVersion?.message} required>
              <ThemedSelect
                value={String(capabilityVersionValue ?? '1')}
