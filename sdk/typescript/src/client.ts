...
export class PromptsheonClient {
  ...
  async listCapabilities(projectId: string): Promise<
    paths["/v1/projects/{project_id}/capabilities"]["get"]["responses"]["200"]["content"]["application/json"]
  > {
    const url = `${this.config.baseUrl}/v1/projects/${encodeURIComponent(projectId)}/capabilities`;
  ...
  async invokeRelease(releaseId: string, body: { inputs: Record<string, unknown> }): Promise<
    paths["/v1/releases/{release_id}/invoke"]["post"]["responses"]["201"]["content"]["application/json"]
  > {
    const url = `${this.config.baseUrl}/v1/releases/${encodeURIComponent(releaseId)}/invoke`;
    const r = await fetch(url, {
      method: "POST",
      headers: {
