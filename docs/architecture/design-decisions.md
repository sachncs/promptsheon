...
See [docs/security.md](../security/security.md#audit-chain).
...
Workflow runtime is in `promptsheon/workflow`.

## Capability Service Level Objectives

Three first-class SLOs ship with the project. See
[docs/slos.md](../operations/slos.md) for the user guide. The Prometheus
...
A deterministic rules engine (`promptsheon/optimizer`)
plus a Thompson Sampling bandit (`promptsheon/bandit`). Both
