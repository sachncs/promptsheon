...
**Anthropic**. Both implement the `promptsheon/llm.Provider`
...
provider, implement the `promptsheon/llm.Provider` interface
...
`promptsheon/llm/circuitbreaker.go` ships a circuit breaker
...
- [docs/security.md](../security/security.md) — Vault + KMS-backed
  `KeyProvider` for the master key, plus the audit chain
  contract.
- [docs/getting-started.md](../development/getting-started.md) — the 10-step
  walkthrough.