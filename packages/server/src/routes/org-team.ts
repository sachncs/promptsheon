...
/**
 * Cedar preHandler factory. Replaces the old role-based
 * `rolePreHandler`; the policy file's role clauses (Role::"admin"
 * etc.) make the same decision, so behavior is preserved for
 * every existing test fixture.
 */
const adminOnly = cedarGate({ action: 'Budget::Create', resource: 'default' });
const adminOrApprover = cedarGate({ action: 'Registry::Sign', resource: 'default' });
...
export function registerOrgTeamRoutes(app: FastifyInstance, deps: {
  ...
  app.post('/api/orgs', { preHandler: adminOnly }, async (request, reply) => {
