...
-- from the existing manifest hash so legacy rows point at a
-- stable commit.
--
-- Migration 027 made repositories.id an alias of projects.id, so
-- repo_id here is just the project_id column mirrored for clarity.
...
-- Backfill repo_id from the legacy project_id (== repo id).
UPDATE capability_versions
   SET repo_id = (
     SELECT c.project_id FROM capabilities c WHERE c.id = capability_versions.capability_id
   );

-- Derive a synthetic commit oid from the manifest hash so the
-- link is non-null even for legacy rows. Stable because both
-- manifest_hash and the derived oid are content-addressed.
UPDATE capability_versions
   SET commit_oid = CASE
