...
/**
 * Merge an incoming draft manifest shape with safe defaults so a
 * partial editor payload (e.g. just {nodes, edges,
 * prompt.systemPrompt}) still validates against ManifestSchema.
 * The activation gate enforces the full shape on promote; this
 * helper exists so the editor's "Save" can persist drafts before
 * every field is filled in.
 *
 * Pass the raw body through `mergeDraftManifest` BEFORE
 * ManifestSchema.parse; the result is guaranteed to satisfy
 * ManifestSchema when the input is at least an object.
 */
export function mergeDraftManifest(input: unknown): Record<string, unknown> {
  ...
  out['prompt'] = {
    systemPrompt:
      typeof prompt['systemPrompt'] === 'string' && prompt['systemPrompt']
        ? prompt['systemPrompt']
