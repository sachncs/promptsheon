...
export function NodeConfigPanel({ selectedNodeId, manifest, onChange }: NodeConfigPanelProps) {
  ...
  if (!node) {
    return (
      <Card>
        <CardHeader><CardTitle className="text-sm">Node Config</CardTitle></CardHeader>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm">Node: <span className="font-mono">{node.id}</span></CardTitle>
      </CardHeader>
      <CardContent className="space-y-3 text-sm">
        ...
        <div>
          ...
          <Textarea
            ...
            onChange={(e) => updateNode({ manifest: { ...node.manifest, prompt: { ...node.manifest.prompt, systemPrompt: e.target.value } } })}
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <Input
              ...
              onChange={(e) => updateNode({ manifest: { ...node.manifest, model: { ...node.manifest.model, modelId: e.target.value } } })}
              placeholder="gpt-4"
            />
          </div>
          <div>
            ...
            <Input
              ...
              onChange={(e) => updateNode({ manifest: { ...node.manifest, model: { ...node.manifest.model, temperature: Number(e.target.value) } } })}
            />
          </div>
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(e) => updateNode({ manifest: { ...node.manifest, runtime: { ...node.manifest.runtime, nodeTimeoutMs: Number(e.target.value) } } })}
          />
        </div>
        <div className="flex items-center gap-2 pt-2">
          <input
            ...
            onChange={(e) => updateNode({ observability: { ...node.observability, logInputs: e.target.checked } })}
          />
