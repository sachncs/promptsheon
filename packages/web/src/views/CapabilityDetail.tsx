...
import { capabilityApi, versionApi, releaseApi, executionApi } from '@/lib/api';
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-4 mt-4">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Card>
              <CardHeader><CardTitle className="text-sm">Releases</CardTitle></CardHeader>
              <CardContent className="text-3xl font-bold">{releases?.length ?? 0}</CardContent>
            </Card>
            <Card>
              <CardHeader><CardTitle className="text-sm">Self-Evolve</CardTitle></CardHeader>
              <CardContent>
        ...
        <TabsContent value="versions" className="mt-4">
          <Card>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow><TableHead>Version</TableHead><TableHead>Created By</TableHead><TableHead>Created At</TableHead></TableRow>
                </TableHeader>
                <TableBody>
                  {versions?.map((v: any) => (
                    <TableRow key={v.id}>
        ...
        <TabsContent value="releases" className="mt-4">
          <Card>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow><TableHead>Environment</TableHead><TableHead>Status</TableHead><TableHead>Created</TableHead></TableRow>
                </TableHeader>
                <TableBody>
                  {releases?.map((r: any) => (
                    <TableRow key={r.id}>
                  ...
                  {(!releases || releases.length === 0) && (
                    <TableRow><TableCell colSpan={3} className="text-center text-muted-foreground">No releases</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </CardContent>
        ...
        <TabsContent value="executions" className="mt-4">
          <Card><CardContent className="text-center text-muted-foreground py-8">Select a version to view executions</CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
  );
