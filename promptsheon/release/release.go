...
// errs.ErrReleaseNotPending is returned when a transition is attempted on a
// Release that is not in the Pending state.

// errs.ErrReleaseUnknownEnvironment is returned when an Environment fails the
// closed-set check.
...
func (r Release) ApproveWith(a approval.Approval, pol approval.Policy) (Release, error) {
	if r.Status != StatusPending {
		return r, errs.ErrReleaseNotPending
	}
	if r.CreatedBy == "" {
