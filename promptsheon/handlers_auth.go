...
// handleCreateAPIKey mints a new API key. Admins may mint keys
// for any role; non-admins receive a self-only writer key.
...
// handleListAPIKeys lists API keys, scoped to the caller's
// permission level: admins see all keys, non-admins see their
// own.
...
// RevokeAPIKey revokes an existing API key by ID. The caller
// must be authenticated; the audit log records the actor.
...
// handleOAuthLogin starts the OAuth login flow by redirecting
// the caller to the provider's authorization URL with a
...
// handleOAuthCallback completes the OAuth login flow. It
// validates the state cookie, exchanges the authorization code,
// and issues an API key for the new user.
