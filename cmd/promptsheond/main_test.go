...
func TestBuildServer_Minimal(t *testing.T) {
	...
	srv, limiter, tracer, collector := buildServer(ctx, &cfg, db, logger, nil, nil, nil, db.DB())
...
func TestBuildServer_WithWebhookEndpointsInDB(t *testing.T) {
	...
	srv, limiter, tracer, collector := buildServer(srvCtx, &cfg, db, logger, nil, nil, nil, db.DB())
	_ = limiter
	_ = tracer
	_ = collector
	if srv == nil {
