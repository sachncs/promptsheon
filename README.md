<p align="center">
  ...
  <p align="center">The Control Plane for AI Capabilities — v1.0.0</p>
  <p align="center">
...
- **REST API** — Full-featured HTTP API; the OpenAPI specification lives at `promptsheon/spec/spec.yaml` (generated by `scripts/genopenapi`, not served at runtime)
...
```bash
# Clone and build (the frontend dist/ is committed; no Node/npm needed
# unless you intend to rebuild the dashboard).
git clone https://github.com/sachncs/promptsheon.git
cd promptsheon
make build                  # produces bin/promptsheond, bin/promptsheon,
                            # bin/promptsheon-healthcheck

# Start the daemon in loopback-only mode with auth disabled. Loopback
# binding is REQUIRED when PROMPTSHEON_AUTH=false: the daemon refuses
# to start on a non-loopback address because POST /api/v1/setup would
# otherwise mint an admin key to any network-adjacent caller.
# PROMPTSHEON_BOOTSTRAP_TOKEN gates the setup endpoint.
export PROMPTSHEON_AUTH=false
export PROMPTSHEON_ADDR=127.0.0.1:8080
export PROMPTSHEON_BOOTSTRAP_TOKEN="$(openssl rand -hex 24)"
./bin/promptsheond
```

In a second terminal, point the CLI at the daemon:

```bash
export PROMPTSHEON_SERVER=http://127.0.0.1:8080
./bin/promptsheon workspace list
./bin/promptsheon provider list
```

### REST API (curl)

```bash
# Bootstrap an admin key. The endpoint returns the plaintext key
# exactly once; store it. The endpoint 404s the moment any user
# record exists.
ADMIN_KEY=$(curl -sS -X POST http://127.0.0.1:8080/api/v1/setup \
  -H "X-Bootstrap-Token: $PROMPTSHEON_BOOTSTRAP_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"email":"admin@example.com","name":"Admin"}' | jq -r .key)
...
curl -sS -X POST http://127.0.0.1:8080/api/v1/workspaces/w1/projects \
  -H "Authorization: Bearer $ADMIN_KEY" \
  -H 'Content-Type: application/json' \
  -d '{"name":"summariser"}'

# Create a Capability under the Project, then a Version with a Manifest.
curl -sS -X POST http://127.0.0.1:8080/api/v1/projects/p1/capabilities \
  -H "Authorization: Bearer $ADMIN_KEY" \
  -H 'Content-Type: application/json' \
  -d '{"name":"summariser","description":"Summarise long docs"}'

curl -sS -X POST http://127.0.0.1:8080/api/v1/capabilities/c1/versions \
  -H "Authorization: Bearer $ADMIN_KEY" \
  -H 'Content-Type: application/json' \
...
REL=$(curl -sS -X POST http://127.0.0.1:8080/api/v1/versions/v1/releases \
        -H "Authorization: Bearer $ADMIN_KEY" \
        -H 'Content-Type: application/json' \
        -d '{"environment":"prod"}' | jq -r .id)
# 2. A distinct identity casts an Approve vote.
curl -sS -X POST http://127.0.0.1:8080/api/v1/releases/$REL/votes \
     -H "Authorization: Bearer $ADMIN_KEY" \
     -H 'Content-Type: application/json' \
...
curl -sS -X POST http://127.0.0.1:8080/api/v1/releases/$REL/activate \
     -H "Authorization: Bearer $ADMIN_KEY"
# 4. Invoke through the configured LLM provider. The Release
#    decides provider + model; the request carries only the inputs.
curl -sS -X POST http://127.0.0.1:8080/api/v1/releases/$REL/invoke \
     -H "Authorization: Bearer $ADMIN_KEY" \
     -H 'Content-Type: application/json' \
     -d '{"inputs":{"q":"hello"}}'
```
...
rel, err := client.CreateRelease(ctx, "v1", promptsheon.CreateReleaseRequest{
    Environment: "prod",
})
if err != nil { return err }

if _, err := client.Vote(ctx, rel.ID, promptsheon.VoteRequest{
    Identity: "alice",
    Decision: "approve",
}); err != nil { return err }
...
out, err := client.Invoke(ctx, rel.ID, promptsheon.InvokeRequest{
    Inputs: map[string]any{"q": "hello"},
})
```

The `pkg/promptsheon` package is the only public Go surface; the
legacy `sdk/` import path was removed in v1.0.0. The package is
gated by the `promptsheon` build tag — consumers must opt in:

```bash
GOFLAGS=-tags=promptsheon go build ./...
GOFLAGS=-tags=promptsheon go test ./pkg/promptsheon/...
```
...
├── pkg/promptsheon/         # Go SDK facade (//go:build promptsheon; pkg/promptsheon/CHANGELOG.md)
...
- [SDK](docs/reference/sdk.md) — Go client (the only SDK that ships today; see PR-4 in docs/research/audit-fixes-plan.md for the deletion of the empty Python/TypeScript directories)
