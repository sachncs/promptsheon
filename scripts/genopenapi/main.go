...
// route registrations in backend/ and the corresponding handler
// functions in backend/handlers_*.go, and emits a real OpenAPI
...
// Usage:
//
//	go run ./scripts/genopenapi                       # write to backend/spec/spec.yaml
...
func main() {
	...
	outPath := flag.String("out", "backend/spec/spec.yaml", "output file (relative to repo root)")
	...
	routes, err := collectRoutesDir(repoRoot + "/backend")
	if err != nil {
	...
	for i := range routes {
		if routes[i].handlerName == "" {
			resolved, resolveErr := resolveAlias(repoRoot+"/backend/routes.go", routes[i])
			if resolveErr != nil {
	...
	handlersByName, err := collectHandlers(repoRoot + "/backend")
	if err != nil {
...
info:
  ...
  # registrations in backend/routes.go and the handler
  # bodies in backend/handlers_*.go. Re-run the generator
