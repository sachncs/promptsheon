...
func main() {
	...
	routes, err := collectRoutesDir(repoRoot + "/backend/api")
	if err != nil {
	...
	for i := range routes {
		if routes[i].handlerName == "" {
			resolved, resolveErr := resolveAlias(repoRoot+"/backend/api/server.go", routes[i])
			if resolveErr != nil {
	...
	handlersByName, err := collectHandlers(repoRoot + "/backend/api")
	if err != nil {
...
info:
  ...
  # registrations in backend/api/server.go and the handler
  # bodies in backend/api/handlers_*.go. Re-run the generator
