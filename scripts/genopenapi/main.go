...
// route registrations in promptsheon/ and the corresponding handler
// functions in promptsheon/handlers_*.go, and emits a real OpenAPI
// path entry per route (no TODO stubs, no template "OK"
// responses).
//
...
// Usage:
//
//	go run ./scripts/genopenapi                       # write to promptsheon/spec/spec.yaml
...
func main() {
	...
	outPath := flag.String("out", "promptsheon/spec/spec.yaml", "output file (relative to repo root)")
	...
	routes, err := collectRoutesDir(repoRoot + "/promptsheon")
	if err != nil {
	...
	// parse routes.go (cheap) or we resolve during collection.
	for i := range routes {
		if routes[i].handlerName == "" {
			resolved, resolveErr := resolveAlias(repoRoot+"/promptsheon/routes.go", routes[i])
			if resolveErr != nil {
	...
	handlersByName, err := collectHandlers(repoRoot + "/promptsheon")
	if err != nil {
...
// in promptsheon/transport.go (system probes, the metrics endpoint)
...
info:
  ...
  # Version %s is the production surface. The spec is
  # auto-generated by scripts/genopenapi from the route
  # registrations in promptsheon/handlers/routes.go and the handler
  # bodies in promptsheon/handlers_*.go. Re-run the generator
  # whenever a route or handler changes.
  version: "%s"
...
func writeRequestBody(buf *bytes.Buffer, rt *requestStruct) {
	...
	for _, f := range rt.Fields {
		...
		if isRef {
			fmt.Fprintf(buf, "                %s:\n", f.JSONName)
			fmt.Fprintf(buf, "                  $ref: \"#/components/schemas/object\"\n")
			continue
		}
		fmt.Fprintf(buf, "                %s:\n", f.JSONName)
