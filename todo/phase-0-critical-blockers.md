...
- [ ] **DB-1a** Rewrite migration 050 in SQLite-compatible form.
...
- [ ] **DB-1b** Update migration test 050 to actually execute 050, not inspect pre-seeded schema.
...
- [ ] **DB-2a** Replace the file-name heuristic with a content-scan that catches `044_legacy_drop`.
...
- [ ] **DB-3a** Move FK-disable pragma outside the migration transaction.
...
- [ ] **OBS-1a** Move `*trace.SQLite.Finish` off the request goroutine.
...
- [ ] **OBS-1b** Export `auditDropped` and `traceDropped` counters to the Prometheus exposition.
  - **Where**: `internal/api/server.go:614, 668-697` and `internal/metrics/collector.go`.

## OTel wiring

- [ ] **OBS-2a** Wire `*trace.OTelTracer` into the HTTP middleware alongside the SQLite tracer.
...
- [ ] **OBS-3a** Fix `OTelTracer.Finish` to call `otelSpan.End()`.
...
- [ ] **OBS-4a** Wrap `ws.streamHandler` into the slog chain in `setupLogger`.
...
- [ ] **OBS-OAuth-1** Construct `*auth.OAuthManager` from env vars in `buildServer`.
...
- [ ] **API-1a** Extend `scripts/genopenapi` to walk every `register*Routes()` method on `*Server`, not just the routes registered in `server.go`.
...
- [ ] **DOC-2a** Set `api/openapi.yaml:version` to `0.1.0` to match the product.
...
- [ ] **OBS-12a** Wire `recommendation.Producer` against the observation aggregator.
...
- [ ] **OBS-Lineage-1** Implement `lineage.Repository` against SQLite.
