...
func RunEvalRunnerHappyPath(t *testing.T) {
	now := time.Now().UTC()
...
func RunEvalRunnerMixedResults(t *testing.T) {
	now := time.Now().UTC()
	r := harness.NewEvalRunner(nil, nil)
	r.Clock = func() time.Time { return now }
	r.Repo = newRepo(t)
...
func RunEvalRunnerInvokerError(t *testing.T) {
	now := time.Now().UTC()
	r := harness.NewEvalRunner(nil, nil)
	r.Clock = func() time.Time { return now }
	r.Repo = newRepo(t)
...
func RunEvalRunnerUnknownScorer(t *testing.T) {
