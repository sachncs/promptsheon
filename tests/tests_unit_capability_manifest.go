...
func RunArtifactRefValid(t *testing.T) {
	t.Parallel()
...
func RunArtifactRefValidationErrors(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name string
		ref  ArtifactRef
...
func RunManifestValid(t *testing.T) {
	t.Parallel()
	m := Manifest{
...
// RunManifestMinimalThree pins MAN-1: a Manifest with only the
...
func RunManifestMinimalThree(t *testing.T) {
	t.Parallel()
	m := Manifest{
		Prompt:        ArtifactRef{Kind: ArtifactPrompt, Hash: goodHash()},
		ModelPolicy:   ArtifactRef{Kind: ArtifactModelPolicy, Hash: goodHash()},
...
func RunManifestEmpty(t *testing.T) {
	t.Parallel()
...
func RunManifestMissingCoreArtifact(t *testing.T) {
	t.Parallel()
	m := Manifest{
		Prompt: ArtifactRef{Kind: ArtifactPrompt, Hash: goodHash()},
	}
...
func RunManifestDuplicateSliceHash(t *testing.T) {
	t.Parallel()
	h := goodHash()
	m := Manifest{
