...
func RunNewDefaultsTick(t *testing.T) {
	t.Parallel()
...
func RunStartStopsOnContextCancel(t *testing.T) {
	t.Parallel()
	repo := &fakeScheduleRepo{}
	pub := eventbus.NewMemory()
	defer pub.Close()
...
func RunTickOncePublishesForDueSchedules(t *testing.T) {
	t.Parallel()
	now := time.Now().UTC()
...
func RunTickOnceListErrorIsSwallowed(t *testing.T) {
	t.Parallel()
...
func RunTickOnceUpdateErrorDoesNotPublish(t *testing.T) {
	t.Parallel()
	now := time.Now().UTC()
	due := &schedule.Schedule{
		ID:          "s1",
