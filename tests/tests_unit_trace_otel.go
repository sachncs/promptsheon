...
// RunOTelSpanLandsInCollector verifies TEST-NEW-12: a request
// produces a span that lands in the in-memory OTLP test collector.
func RunOTelSpanLandsInCollector(t *testing.T) {
...
// RunOTelSpanAttributes verifies attributes set on a span
// are collected.
func RunOTelSpanAttributes(t *testing.T) {
	collector := testutil.NewInMemoryCollector()
	defer collector.Shutdown(context.Background())
...
// RunOTelChildSpans verifies parent-child span relationship.
func RunOTelChildSpans(t *testing.T) {
	collector := testutil.NewInMemoryCollector()
	defer collector.Shutdown(context.Background())
...
// RunConcurrentTraceWrites verifies TEST-NEW-9: 1000
...
func RunConcurrentTraceWrites(t *testing.T) {
	collector := testutil.NewInMemoryCollector()
	defer collector.Shutdown(context.Background())
