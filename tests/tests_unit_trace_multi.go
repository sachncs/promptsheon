...
func RunMultiDispatchesAll(t *testing.T) {
	a := &countingTracer{}
	b := &countingTracer{}
	m := NewMulti(a, b)
...
func RunMultiWithOnlyPrimary(t *testing.T) {
	a := &countingTracer{}
	m := NewMulti(a)
	span := m.Start(context.Background(), "op")
	_ = m.Finish(span)
...
func RunMultiAllNilReturnsNoop(t *testing.T) {
...
func RunMultiFallbackWhenPrimaryNil(t *testing.T) {
	a := &countingTracer{}
	m := NewMulti(nil, a)
	span := m.Start(context.Background(), "op")
	_ = m.Finish(span)
