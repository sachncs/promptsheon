...
func RunCompileMatchesByGoalToken(t *testing.T) {
	t.Parallel()
...
func RunCompileNoMatch(t *testing.T) {
	t.Parallel()
	c := NewCompiler(catalog())
...
func RunCompileEmptyGoalRejected(t *testing.T) {
	t.Parallel()
	c := NewCompiler(catalog())
...
func RunCompileRespectsMinTrustScore(t *testing.T) {
	t.Parallel()
	c := NewCompiler(catalog())
	_, err := c.Compile(context.Background(), Intent{
		ID:   "i1",
...
func RunCompileRespectsMaxCost(t *testing.T) {
	t.Parallel()
	c := NewCompiler(catalog())
	_, err := c.Compile(context.Background(), Intent{
		ID:   "i1",
...
func RunCompileRespectsRequiredTags(t *testing.T) {
	t.Parallel()
	c := NewCompiler(catalog())
	_, err := c.Compile(context.Background(), Intent{
		ID:   "i1",
...
func RunCompilePicksCheaperOnTie(t *testing.T) {
	t.Parallel()
...
func RunCompilePlanIDAndBudget(t *testing.T) {
	t.Parallel()
	c := NewCompiler(catalog())
	before := time.Now().UTC()
...
func RunCompileContextCancel(t *testing.T) {
	t.Parallel()
	c := NewCompiler(catalog())
