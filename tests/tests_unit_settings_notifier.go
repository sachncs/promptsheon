...
func RunNotifier_PublishReturnsSubscriberError(t *testing.T) {
	t.Parallel()
	n := NewNotifier()
...
func RunNotifier_PublishStopsOnFirstError(t *testing.T) {
	t.Parallel()
	n := NewNotifier()
...
func RunNotifier_VaultReloadFailurePropagates(t *testing.T) {
	t.Parallel()
...
func RunNotifier_NoSubscribers(t *testing.T) {
	t.Parallel()
	n := NewNotifier()
	if err := n.Publish("vault.key", "x"); err != nil {
...
func RunNotifier_MultipleSubscribersAllRun(t *testing.T) {
	t.Parallel()
	n := NewNotifier()
	calls := 0
