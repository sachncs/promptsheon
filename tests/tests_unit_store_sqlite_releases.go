...
func RunReleaseCreateGetRoundTrip(t *testing.T) {
...
func RunReleaseActivateSupersedes(t *testing.T) {
	fx := newReleaseFixture(t)
	ctx := context.Background()
	now := time.Now().UTC()
...
func RunApprovalRoundTrip(t *testing.T) {
	fx := newReleaseFixture(t)
	ctx := context.Background()
	now := time.Now().UTC()
...
// RunActivateAtomicRollbackOnMissingNext verifies that ActivateAtomic
// returns ErrNotFound and rolls back when the next Release ID is not
// present in the table. The prior must not be mutated.
func RunActivateAtomicRollbackOnMissingNext(t *testing.T) {
	fx := newReleaseFixture(t)
	ctx := context.Background()
