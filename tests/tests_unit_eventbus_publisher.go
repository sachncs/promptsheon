...
func RunPublisherFiltersByType(t *testing.T) {
	t.Parallel()
	p := NewMemory()
	defer p.Close()
...
func RunPublisherReceivesAllWhenFilterEmpty(t *testing.T) {
	t.Parallel()
	p := NewMemory()
	defer p.Close()
...
func RunSubscribeRejectsNilHandler(t *testing.T) {
	t.Parallel()
	p := NewMemory()
	defer p.Close()
...
func RunCancelStopsDelivery(t *testing.T) {
	t.Parallel()
	p := NewMemory()
	defer p.Close()
...
func RunSubscribeRejectsDuplicateSubscription(t *testing.T) {
	t.Parallel()
...
func RunPublishPanicRecovered(t *testing.T) {
	t.Parallel()
	p := NewMemory()
	defer p.Close()
...
// RunAsyncMemoryPublishDoesNotBlock covers OBS-5b: Publish on
...
func RunAsyncMemoryPublishDoesNotBlock(t *testing.T) {
	t.Parallel()
...
// RunAsyncMemoryDropCounter covers OBS-5b: the dropped counter
// starts at zero and increments only on full-queue drops.
func RunAsyncMemoryDropCounter(t *testing.T) {
	t.Parallel()
	m := NewAsyncMemory(64, 4)
	defer m.Close()
