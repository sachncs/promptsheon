...
// RunMergeDominance asserts that when one vector strictly
// dominates the other, the dominant record wins.
func RunMergeDominance(t *testing.T) {
	t.Parallel()
...
// RunMergeTimestampBeatsTieBreak asserts that a higher
// WriteTS wins regardless of replica id when the vectors are
// incomparable.
func RunMergeTimestampBeatsTieBreak(t *testing.T) {
	t.Parallel()
	a := makeRecord("k", "old", "rep-a", 100, map[string]uint64{"rep-a": 1}, false)
...
// RunMergeACI asserts associativity, commutativity, and
...
// RunMergeACIWellFormed asserts associativity on
...
func RunMergeACIWellFormed(t *testing.T) {
	t.Parallel()
	rng := rand.New(rand.NewPCG(1, 1))
	replicas := []string{"rep-a", "rep-b", "rep-c"}
	for i := 0; i < 500; i++ {
...
// RunMergeTombstoneWinsExactMetadataTie pins the
...
// tombstoned key.
func RunMergeTombstoneWinsExactMetadataTie(t *testing.T) {
	t.Parallel()
...
// RunMergeDivergentPayloadIsCommutative exercises the
...
func RunMergeDivergentPayloadIsCommutative(t *testing.T) {
	t.Parallel()
	vec := map[string]uint64{"rep-a": 1}
	ts := int64(100)
	replica := "rep-a"
...
// RunTombstoneHidesRecord asserts that a tombstoned record
...
func RunTombstoneHidesRecord(t *testing.T) {
	t.Parallel()
...
// RunSetCanResurrectAfterDominance asserts the no-resurrection
// rule: a Set on a tombstoned key only sticks if the new
// write dominates the tombstone's vector.
func RunSetCanResurrectAfterDominance(t *testing.T) {
	t.Parallel()
	deleted := makeRecord("k", "", "rep-a", 100, map[string]uint64{"rep-a": 2}, true)
...
// RunVectorDominatedUnit pins the VectorDominated function:
// a must be >= b in every coordinate with strict inequality
// somewhere.
func RunVectorDominatedUnit(t *testing.T) {
	t.Parallel()
...
// RunIncVector does not mutate the input.
func RunIncVector(t *testing.T) {
	t.Parallel()
...
// RunIncVectorNewReplica: bumping an unseen replica starts
// at 1.
func RunIncVectorNewReplica(t *testing.T) {
	t.Parallel()
...
// RunMergeRecordsEmpty pins the degenerate case.
func RunMergeRecordsEmpty(t *testing.T) {
	t.Parallel()
...
// RunMergeRecordsSingle pins the single-element case (the
// fold must be a no-op).
func RunMergeRecordsSingle(t *testing.T) {
	t.Parallel()
...
// RunConcurrentReplicaConvergence: two replicas write
// disjoint vectors to the same key; after exchanging
// records, they converge to the same CRDTRecord.
func RunConcurrentReplicaConvergence(t *testing.T) {
	t.Parallel()
...
// RunTombstoneDoesNotResurrectUnderEqualVector: a Set on a
...
func RunTombstoneDoesNotResurrectUnderEqualVector(t *testing.T) {
	t.Parallel()
...
// RunMergeFoldsVectorEntries is the regression for an old
...
func RunMergeFoldsVectorEntries(t *testing.T) {
	t.Parallel()
