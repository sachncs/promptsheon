...
func RunDefaultConfig(t *testing.T) {
	cfg := DefaultConfig()
...
func RunLoadConfigReturnsFileError(t *testing.T) {
...
func RunLoadConfigFromEnv(t *testing.T) {
...
func RunLoadConfigAuthValues(t *testing.T) {
	tests := []struct {
		name     string
		value    string
		expected bool
...
// RunLoadConfig_InvalidNumericWarns pins the M-4 fix: an invalid
...
func RunLoadConfig_InvalidNumericWarns(t *testing.T) {
...
func RunPortServiceName(t *testing.T) {
...
func RunLoadConfig_AdditionalEnvs(t *testing.T) {
...
func RunSanitizeConfigClampsNegative(t *testing.T) {
	prev := slog.Default()
	defer slog.SetDefault(prev)
	var buf bytes.Buffer
	slog.SetDefault(slog.New(slog.NewJSONHandler(&buf, &slog.HandlerOptions{Level: slog.LevelWarn})))
